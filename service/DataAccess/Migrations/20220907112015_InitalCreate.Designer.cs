// <auto-generated />
using System;
using DataAccess.Concrete.EntityFramework.Contexts;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace DataAccess.Migrations
{
    [DbContext(typeof(HubContext))]
    [Migration("20220907112015_InitalCreate")]
    partial class InitalCreate
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("Core.Entity.Concrete.OperationClaim", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<string>("Name")
                        .HasColumnType("longtext");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.ToTable("OperationClaims");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Created = 0L,
                            Name = "Student",
                            Updated = 0L
                        },
                        new
                        {
                            Id = 2,
                            Created = 0L,
                            Name = "Business",
                            Updated = 0L
                        },
                        new
                        {
                            Id = 3,
                            Created = 0L,
                            Name = "Admin",
                            Updated = 0L
                        });
                });

            modelBuilder.Entity("Core.Entity.Concrete.UserOperationClaim", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<int>("OperationClaimId")
                        .HasColumnType("int");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("UserOperationClaims");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Created = 0L,
                            OperationClaimId = 3,
                            Updated = 0L,
                            UserId = 1
                        },
                        new
                        {
                            Id = 2,
                            Created = 0L,
                            OperationClaimId = 1,
                            Updated = 0L,
                            UserId = 2
                        });
                });

            modelBuilder.Entity("Entity.Concrete.Comment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("AuthorId")
                        .HasColumnType("int");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<int>("PostId")
                        .HasColumnType("int");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("AuthorId");

                    b.HasIndex("PostId");

                    b.ToTable("Comments");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            AuthorId = 1,
                            Content = "Güzel gönderi",
                            Created = 1661243520L,
                            PostId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 2,
                            AuthorId = 1,
                            Content = "Tanışmak ister misin?",
                            Created = 1661243520L,
                            PostId = 1,
                            Updated = 1661243520L
                        });
                });

            modelBuilder.Entity("Entity.Concrete.Department", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<int>("DepartmentCodeId")
                        .HasColumnType("int");

                    b.Property<int>("FacultyId")
                        .HasColumnType("int");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("DepartmentCodeId");

                    b.HasIndex("FacultyId");

                    b.ToTable("Departments");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Created = 1661243520L,
                            DepartmentCodeId = 1,
                            FacultyId = 10,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 2,
                            Created = 1661243520L,
                            DepartmentCodeId = 2,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 3,
                            Created = 1661243520L,
                            DepartmentCodeId = 3,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 4,
                            Created = 1661243520L,
                            DepartmentCodeId = 4,
                            FacultyId = 3,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 5,
                            Created = 1661243520L,
                            DepartmentCodeId = 5,
                            FacultyId = 10,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 6,
                            Created = 1661243520L,
                            DepartmentCodeId = 6,
                            FacultyId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 7,
                            Created = 1661243520L,
                            DepartmentCodeId = 7,
                            FacultyId = 3,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 8,
                            Created = 1661243520L,
                            DepartmentCodeId = 8,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 9,
                            Created = 1661243520L,
                            DepartmentCodeId = 9,
                            FacultyId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 10,
                            Created = 1661243520L,
                            DepartmentCodeId = 10,
                            FacultyId = 3,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 11,
                            Created = 1661243520L,
                            DepartmentCodeId = 11,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 12,
                            Created = 1661243520L,
                            DepartmentCodeId = 12,
                            FacultyId = 5,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 13,
                            Created = 1661243520L,
                            DepartmentCodeId = 13,
                            FacultyId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 14,
                            Created = 1661243520L,
                            DepartmentCodeId = 14,
                            FacultyId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 15,
                            Created = 1661243520L,
                            DepartmentCodeId = 15,
                            FacultyId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 16,
                            Created = 1661243520L,
                            DepartmentCodeId = 16,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 17,
                            Created = 1661243520L,
                            DepartmentCodeId = 17,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 18,
                            Created = 1661243520L,
                            DepartmentCodeId = 18,
                            FacultyId = 3,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 19,
                            Created = 1661243520L,
                            DepartmentCodeId = 19,
                            FacultyId = 10,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 20,
                            Created = 1661243520L,
                            DepartmentCodeId = 20,
                            FacultyId = 4,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 21,
                            Created = 1661243520L,
                            DepartmentCodeId = 21,
                            FacultyId = 5,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 22,
                            Created = 1661243520L,
                            DepartmentCodeId = 22,
                            FacultyId = 6,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 23,
                            Created = 1661243520L,
                            DepartmentCodeId = 23,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 24,
                            Created = 1661243520L,
                            DepartmentCodeId = 24,
                            FacultyId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 25,
                            Created = 1661243520L,
                            DepartmentCodeId = 25,
                            FacultyId = 5,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 26,
                            Created = 1661243520L,
                            DepartmentCodeId = 26,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 27,
                            Created = 1661243520L,
                            DepartmentCodeId = 27,
                            FacultyId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 28,
                            Created = 1661243520L,
                            DepartmentCodeId = 28,
                            FacultyId = 5,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 29,
                            Created = 1661243520L,
                            DepartmentCodeId = 29,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 30,
                            Created = 1661243520L,
                            DepartmentCodeId = 30,
                            FacultyId = 7,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 31,
                            Created = 1661243520L,
                            DepartmentCodeId = 31,
                            FacultyId = 7,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 32,
                            Created = 1661243520L,
                            DepartmentCodeId = 32,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 33,
                            Created = 1661243520L,
                            DepartmentCodeId = 33,
                            FacultyId = 5,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 34,
                            Created = 1661243520L,
                            DepartmentCodeId = 34,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 35,
                            Created = 1661243520L,
                            DepartmentCodeId = 35,
                            FacultyId = 3,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 36,
                            Created = 1661243520L,
                            DepartmentCodeId = 36,
                            FacultyId = 3,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 37,
                            Created = 1661243520L,
                            DepartmentCodeId = 37,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 38,
                            Created = 1661243520L,
                            DepartmentCodeId = 38,
                            FacultyId = 3,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 39,
                            Created = 1661243520L,
                            DepartmentCodeId = 39,
                            FacultyId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 40,
                            Created = 1661243520L,
                            DepartmentCodeId = 40,
                            FacultyId = 8,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 41,
                            Created = 1661243520L,
                            DepartmentCodeId = 41,
                            FacultyId = 3,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 42,
                            Created = 1661243520L,
                            DepartmentCodeId = 42,
                            FacultyId = 2,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 43,
                            Created = 1661243520L,
                            DepartmentCodeId = 43,
                            FacultyId = 5,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 44,
                            Created = 1661243520L,
                            DepartmentCodeId = 44,
                            FacultyId = 9,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 45,
                            Created = 1661243520L,
                            DepartmentCodeId = 45,
                            FacultyId = 3,
                            Updated = 1661243520L
                        });
                });

            modelBuilder.Entity("Entity.Concrete.DepartmentCode", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.ToTable("DepartmentCodes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Created = 1661243520L,
                            Name = "Acil Yardım ve Afet Yönetimi",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 2,
                            Created = 1661243520L,
                            Name = "Alman Dili ve Edebiyatı",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 3,
                            Created = 1661243520L,
                            Name = "Arkeoloji",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 4,
                            Created = 1661243520L,
                            Name = "Bahçe Bitkileri",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 5,
                            Created = 1661243520L,
                            Name = "Beslenme ve Diyetetik",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 6,
                            Created = 1661243520L,
                            Name = "Bilgisayar Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 7,
                            Created = 1661243520L,
                            Name = "Bitki Koruma",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 8,
                            Created = 1661243520L,
                            Name = "Biyoloji",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 9,
                            Created = 1661243520L,
                            Name = "Biyomedikal Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 10,
                            Created = 1661243520L,
                            Name = "Biyosistem Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 11,
                            Created = 1661243520L,
                            Name = "Coğrafya",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 12,
                            Created = 1661243520L,
                            Name = "Çalışma Ekonomisi ve Endüstri İlişkileri",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 13,
                            Created = 1661243520L,
                            Name = "Çevre Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 14,
                            Created = 1661243520L,
                            Name = "Elektrik-Elektronik Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 15,
                            Created = 1661243520L,
                            Name = "Endüstri Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 16,
                            Created = 1661243520L,
                            Name = "Fizik",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 17,
                            Created = 1661243520L,
                            Name = "Fransız Dili ve Edebiyatı",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 18,
                            Created = 1661243520L,
                            Name = "Gıda Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 19,
                            Created = 1661243520L,
                            Name = "Hemşirelik",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 20,
                            Created = 1661243520L,
                            Name = "Hukuk",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 21,
                            Created = 1661243520L,
                            Name = "İktisat",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 22,
                            Created = 1661243520L,
                            Name = "İlahiyat",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 23,
                            Created = 1661243520L,
                            Name = "İngiliz Dili ve Edebiyatı",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 24,
                            Created = 1661243520L,
                            Name = "İnşaat Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 25,
                            Created = 1661243520L,
                            Name = "İşletme",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 26,
                            Created = 1661243520L,
                            Name = "Kimya",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 27,
                            Created = 1661243520L,
                            Name = "Makine Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 28,
                            Created = 1661243520L,
                            Name = "Maliye",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 29,
                            Created = 1661243520L,
                            Name = "Matematik",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 30,
                            Created = 1661243520L,
                            Name = "Mimarlık",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 31,
                            Created = 1661243520L,
                            Name = "Peyzaj Mimarlığı",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 32,
                            Created = 1661243520L,
                            Name = "Psikoloji",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 33,
                            Created = 1661243520L,
                            Name = "Siyaset Bilimi ve Kamu Yönetimi",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 34,
                            Created = 1661243520L,
                            Name = "Sosyoloji",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 35,
                            Created = 1661243520L,
                            Name = "Tarım Ekonomisi",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 36,
                            Created = 1661243520L,
                            Name = "Tarımsal Biyoteknoloji",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 37,
                            Created = 1661243520L,
                            Name = "Tarih",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 38,
                            Created = 1661243520L,
                            Name = "Tarla Bitkileri",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 39,
                            Created = 1661243520L,
                            Name = "Tekstil Mühendisliği",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 40,
                            Created = 1661243520L,
                            Name = "Tıp",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 41,
                            Created = 1661243520L,
                            Name = "Toprak Bilimi ve Bitki Besleme",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 42,
                            Created = 1661243520L,
                            Name = "Türk Dili ve Edebiyatı",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 43,
                            Created = 1661243520L,
                            Name = "Uluslararası İlişkiler",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 44,
                            Created = 1661243520L,
                            Name = "Veterinerlik",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 45,
                            Created = 1661243520L,
                            Name = "Zootekni",
                            Updated = 1661243520L
                        });
                });

            modelBuilder.Entity("Entity.Concrete.Faculty", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<double>("Altitude")
                        .HasColumnType("double");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<double>("Latitude")
                        .HasColumnType("double");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<int>("UniversityId")
                        .HasColumnType("int");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("UniversityId");

                    b.ToTable("Faculties");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "Çorlu Mühendislik Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 2,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "Fen-Edebiyat Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 3,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "Ziraat Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 4,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "Hukuk Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 5,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "İktisadi ve İdari Bilimler Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 6,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "İlahiyat Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 7,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "Güzel Sanatlar, Tasarım ve Mimarlık Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 8,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "Tıp Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 9,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "Veteriner Fakültesi",
                            UniversityId = 1,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 10,
                            Address = "Çorlu, silahtarağa mah.",
                            Altitude = 21.213124000000001,
                            Created = 1661243520L,
                            Latitude = 43.213411999999998,
                            Name = "Sağlık Yüksekokulu",
                            UniversityId = 1,
                            Updated = 1661243520L
                        });
                });

            modelBuilder.Entity("Entity.Concrete.Post", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("AuthorId")
                        .HasColumnType("int");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("AuthorId");

                    b.ToTable("Posts");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            AuthorId = 1,
                            Content = "I used to live in my neighbor's fishpond, but the aesthetic wasn't to my taste.",
                            Created = 1661243520L,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 2,
                            AuthorId = 1,
                            Content = "The waitress was not amused when he ordered green eggs and ham.",
                            Created = 1661243520L,
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 3,
                            AuthorId = 1,
                            Content = "I don’t respect anybody who can’t tell the difference between Pepsi and Coke.",
                            Created = 1661243520L,
                            Updated = 1661243520L
                        });
                });

            modelBuilder.Entity("Entity.Concrete.PostHasTag", b =>
                {
                    b.Property<int>("PostId")
                        .HasColumnType("int");

                    b.Property<int>("TagId")
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("PostId", "TagId");

                    b.HasIndex("TagId");

                    b.ToTable("PostHasTag");

                    b.HasData(
                        new
                        {
                            PostId = 1,
                            TagId = 1,
                            Created = 0L,
                            Updated = 0L
                        },
                        new
                        {
                            PostId = 1,
                            TagId = 2,
                            Created = 0L,
                            Updated = 0L
                        },
                        new
                        {
                            PostId = 2,
                            TagId = 3,
                            Created = 0L,
                            Updated = 0L
                        },
                        new
                        {
                            PostId = 2,
                            TagId = 4,
                            Created = 0L,
                            Updated = 0L
                        },
                        new
                        {
                            PostId = 3,
                            TagId = 5,
                            Created = 0L,
                            Updated = 0L
                        },
                        new
                        {
                            PostId = 3,
                            TagId = 1,
                            Created = 0L,
                            Updated = 0L
                        });
                });

            modelBuilder.Entity("Entity.Concrete.Tag", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<string>("Description")
                        .HasColumnType("longtext");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.ToTable("Tags");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Created = 1661243520L,
                            Description = "Sadece goygoy. Ciddi olma!",
                            Name = "Goygoy",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 2,
                            Created = 1661243520L,
                            Description = "Kedini burada paylaşabilirsin.",
                            Name = "Hayvani",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 3,
                            Created = 1661243520L,
                            Description = "İkinci el ürününü hemen elinden çıkart.",
                            Name = "İkinci el",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 4,
                            Created = 1661243520L,
                            Description = "Hadi tanışalım.",
                            Name = "Tanışma",
                            Updated = 1661243520L
                        },
                        new
                        {
                            Id = 5,
                            Created = 1661243520L,
                            Description = "Nerelerde takılıyorsun?",
                            Name = "Mekanlar",
                            Updated = 1661243520L
                        });
                });

            modelBuilder.Entity("Entity.Concrete.University", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("City")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<int>("FoundationYear")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.ToTable("Universities");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            City = "29",
                            Created = 1661243520L,
                            FoundationYear = 2000,
                            Name = "Tekirdağ Namık Kemal Üniversitesi",
                            Updated = 1661243520L
                        });
                });

            modelBuilder.Entity("Entity.Concrete.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<int?>("DepartmentId")
                        .HasColumnType("int");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<int?>("FacultyId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<byte[]>("PasswordHash")
                        .HasColumnType("longblob");

                    b.Property<byte[]>("PasswordSalt")
                        .HasColumnType("longblob");

                    b.Property<string>("Role")
                        .HasColumnType("longtext");

                    b.Property<bool>("Status")
                        .HasColumnType("tinyint(1)");

                    b.Property<int?>("UniversityId")
                        .HasColumnType("int");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("DepartmentId");

                    b.HasIndex("FacultyId");

                    b.HasIndex("UniversityId");

                    b.ToTable("Users");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Created = 0L,
                            Email = "berkslv@gmail.com",
                            Name = "Berk Selvi",
                            PasswordHash = new byte[] { 82, 222, 203, 108, 238, 198, 200, 98, 180, 0, 12, 84, 219, 78, 82, 29, 181, 94, 221, 253, 171, 180, 20, 134, 222, 197, 106, 127, 172, 253, 187, 30, 182, 36, 67, 76, 161, 16, 146, 177, 14, 213, 203, 193, 227, 59, 19, 165, 242, 187, 246, 108, 57, 141, 64, 182, 164, 189, 66, 248, 139, 213, 159, 153 },
                            PasswordSalt = new byte[] { 63, 151, 132, 111, 151, 182, 213, 5, 78, 46, 97, 173, 217, 220, 75, 17, 235, 202, 208, 36, 69, 167, 124, 2, 178, 31, 174, 86, 23, 91, 71, 219, 99, 242, 104, 121, 10, 34, 164, 153, 75, 150, 182, 6, 121, 231, 71, 217, 146, 116, 135, 191, 235, 151, 33, 119, 0, 175, 25, 73, 208, 114, 52, 168, 3, 95, 221, 37, 103, 192, 102, 248, 129, 94, 147, 170, 115, 171, 138, 216, 252, 237, 164, 70, 144, 30, 170, 202, 64, 1, 108, 187, 247, 158, 225, 215, 193, 23, 62, 51, 58, 165, 62, 79, 45, 139, 92, 66, 153, 141, 43, 254, 227, 42, 169, 148, 177, 131, 19, 240, 42, 138, 229, 157, 215, 112, 41, 235 },
                            Role = "Admin",
                            Status = true,
                            Updated = 0L,
                            Username = "berkslv"
                        },
                        new
                        {
                            Id = 2,
                            Created = 0L,
                            DepartmentId = 1,
                            Email = "test@example.com",
                            FacultyId = 1,
                            Name = "Deneme kullanıcısı",
                            PasswordHash = new byte[] { 71, 18, 58, 33, 27, 21, 197, 107, 198, 57, 46, 196, 123, 135, 90, 222, 200, 143, 34, 146, 120, 102, 124, 218, 186, 227, 71, 227, 195, 199, 36, 204, 231, 57, 200, 208, 212, 85, 71, 21, 6, 88, 162, 236, 230, 30, 63, 36, 149, 124, 37, 221, 183, 221, 98, 172, 132, 99, 103, 122, 140, 41, 45, 198 },
                            PasswordSalt = new byte[] { 174, 42, 149, 29, 209, 252, 123, 237, 109, 211, 53, 43, 197, 178, 198, 93, 4, 6, 189, 114, 245, 45, 45, 16, 106, 186, 14, 99, 88, 200, 92, 169, 208, 44, 12, 189, 170, 102, 127, 191, 144, 94, 97, 241, 215, 169, 138, 167, 146, 42, 200, 247, 121, 16, 203, 241, 143, 135, 182, 120, 138, 231, 123, 102, 165, 149, 24, 246, 240, 205, 204, 71, 204, 138, 103, 227, 26, 213, 170, 76, 170, 231, 36, 247, 160, 103, 199, 27, 26, 132, 6, 175, 12, 47, 89, 205, 107, 104, 106, 157, 9, 164, 176, 169, 81, 124, 130, 47, 69, 154, 106, 33, 199, 165, 234, 28, 37, 163, 217, 61, 121, 134, 197, 205, 252, 118, 86, 207 },
                            Role = "Student",
                            Status = true,
                            UniversityId = 1,
                            Updated = 0L,
                            Username = "testuser"
                        });
                });

            modelBuilder.Entity("Entity.Concrete.UserCode", b =>
                {
                    b.Property<int>("UserCodeId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Code")
                        .HasColumnType("longtext");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("UserCodeId");

                    b.HasIndex("UserId");

                    b.ToTable("UserCodes");
                });

            modelBuilder.Entity("Entity.Concrete.UserDislikeComment", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int>("CommentId")
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("UserId", "CommentId");

                    b.HasIndex("CommentId");

                    b.ToTable("UserDislikeComment");
                });

            modelBuilder.Entity("Entity.Concrete.UserDislikePost", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int>("PostId")
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("UserId", "PostId");

                    b.HasIndex("PostId");

                    b.ToTable("UserDislikePost");
                });

            modelBuilder.Entity("Entity.Concrete.UserLikeComment", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int>("CommentId")
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("UserId", "CommentId");

                    b.HasIndex("CommentId");

                    b.ToTable("UserLikeComment");
                });

            modelBuilder.Entity("Entity.Concrete.UserLikePost", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int>("PostId")
                        .HasColumnType("int");

                    b.Property<long>("Created")
                        .HasColumnType("bigint");

                    b.Property<long>("Updated")
                        .HasColumnType("bigint");

                    b.HasKey("UserId", "PostId");

                    b.HasIndex("PostId");

                    b.ToTable("UserLikePost");
                });

            modelBuilder.Entity("Entity.Concrete.Comment", b =>
                {
                    b.HasOne("Entity.Concrete.User", "Author")
                        .WithMany("Comments")
                        .HasForeignKey("AuthorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Entity.Concrete.Post", "Post")
                        .WithMany("Comments")
                        .HasForeignKey("PostId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Author");

                    b.Navigation("Post");
                });

            modelBuilder.Entity("Entity.Concrete.Department", b =>
                {
                    b.HasOne("Entity.Concrete.DepartmentCode", "DepartmentCode")
                        .WithMany("Departments")
                        .HasForeignKey("DepartmentCodeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Entity.Concrete.Faculty", "Faculty")
                        .WithMany("Departments")
                        .HasForeignKey("FacultyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("DepartmentCode");

                    b.Navigation("Faculty");
                });

            modelBuilder.Entity("Entity.Concrete.Faculty", b =>
                {
                    b.HasOne("Entity.Concrete.University", "University")
                        .WithMany("Faculties")
                        .HasForeignKey("UniversityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("University");
                });

            modelBuilder.Entity("Entity.Concrete.Post", b =>
                {
                    b.HasOne("Entity.Concrete.User", "Author")
                        .WithMany("Posts")
                        .HasForeignKey("AuthorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Author");
                });

            modelBuilder.Entity("Entity.Concrete.PostHasTag", b =>
                {
                    b.HasOne("Entity.Concrete.Post", "Post")
                        .WithMany("Tags")
                        .HasForeignKey("PostId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Entity.Concrete.Tag", "Tag")
                        .WithMany("Posts")
                        .HasForeignKey("TagId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Post");

                    b.Navigation("Tag");
                });

            modelBuilder.Entity("Entity.Concrete.User", b =>
                {
                    b.HasOne("Entity.Concrete.Department", "Department")
                        .WithMany("Users")
                        .HasForeignKey("DepartmentId");

                    b.HasOne("Entity.Concrete.Faculty", "Faculty")
                        .WithMany("Users")
                        .HasForeignKey("FacultyId");

                    b.HasOne("Entity.Concrete.University", "University")
                        .WithMany("Users")
                        .HasForeignKey("UniversityId");

                    b.Navigation("Department");

                    b.Navigation("Faculty");

                    b.Navigation("University");
                });

            modelBuilder.Entity("Entity.Concrete.UserCode", b =>
                {
                    b.HasOne("Entity.Concrete.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Entity.Concrete.UserDislikeComment", b =>
                {
                    b.HasOne("Entity.Concrete.Comment", "Comment")
                        .WithMany("Dislikes")
                        .HasForeignKey("CommentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Entity.Concrete.User", "User")
                        .WithMany("DislikedComments")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Comment");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Entity.Concrete.UserDislikePost", b =>
                {
                    b.HasOne("Entity.Concrete.Post", "Post")
                        .WithMany("Dislikes")
                        .HasForeignKey("PostId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Entity.Concrete.User", "User")
                        .WithMany("DislikedPosts")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Post");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Entity.Concrete.UserLikeComment", b =>
                {
                    b.HasOne("Entity.Concrete.Comment", "Comment")
                        .WithMany("Likes")
                        .HasForeignKey("CommentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Entity.Concrete.User", "User")
                        .WithMany("LikedComments")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Comment");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Entity.Concrete.UserLikePost", b =>
                {
                    b.HasOne("Entity.Concrete.Post", "Post")
                        .WithMany("Likes")
                        .HasForeignKey("PostId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Entity.Concrete.User", "User")
                        .WithMany("LikedPosts")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Post");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Entity.Concrete.Comment", b =>
                {
                    b.Navigation("Dislikes");

                    b.Navigation("Likes");
                });

            modelBuilder.Entity("Entity.Concrete.Department", b =>
                {
                    b.Navigation("Users");
                });

            modelBuilder.Entity("Entity.Concrete.DepartmentCode", b =>
                {
                    b.Navigation("Departments");
                });

            modelBuilder.Entity("Entity.Concrete.Faculty", b =>
                {
                    b.Navigation("Departments");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("Entity.Concrete.Post", b =>
                {
                    b.Navigation("Comments");

                    b.Navigation("Dislikes");

                    b.Navigation("Likes");

                    b.Navigation("Tags");
                });

            modelBuilder.Entity("Entity.Concrete.Tag", b =>
                {
                    b.Navigation("Posts");
                });

            modelBuilder.Entity("Entity.Concrete.University", b =>
                {
                    b.Navigation("Faculties");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("Entity.Concrete.User", b =>
                {
                    b.Navigation("Comments");

                    b.Navigation("DislikedComments");

                    b.Navigation("DislikedPosts");

                    b.Navigation("LikedComments");

                    b.Navigation("LikedPosts");

                    b.Navigation("Posts");
                });
#pragma warning restore 612, 618
        }
    }
}
